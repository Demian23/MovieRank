...
extension FavouritesViewModel {
    ...
    public func clearCurrentState() {
        favouritesListener?.remove()
        movies.removeAll()
        favouritesListener = nil
    }

    public func localUpdate(newMovie: Movie) {
    ...
    @MainActor
    public func handleRealtimeChangesInFavourites(uid: String, movieListVM: MovieListViewModel) {
            ...
            favouritesListener = currentFavouritesRef.addSnapshotListener { querySnapshot, error in
                ...
                snapshot.documentChanges.forEach { diff in
                    if diff.type == .added {
                        ...
                        if let index = movieListVM.movies.firstIndex(where: { movie in
                            movie.id == movieId
                        }) {
                            ...
                            Task {
                                guard var movie = try await MovieConnector.getMovie(by: movieId)
                                else { return }
                                movie.favouritesProperties = favProp
                                self.movies.append(movie)
                            }
                    ...
                    if diff.type == .modified {
                        let movieId = diff.document.documentID
                        let favProp = try! diff.document.data(as: FavouritesProperties.self)
                        if let index = self.movies.firstIndex(where: { favMovie in
                            favMovie.id == movieId
                        }) {
