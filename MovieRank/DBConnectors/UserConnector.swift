...
class UserConnector {
    ...
    static func deleteUserData(userId uid: String) async throws {
        try await UserConnector.userDocumentRef(for: uid).delete()
        // Delete all marks ? or use cloud function
    }
    
    static func setNewUserScore(userId uid: String, userScore score: Int) async throws {
        try await UserConnector.userDocumentRef(for: uid).updateData([UserConnector.userScoreKey: score])
    }
    static func changeUserScore(userId uid: String, on: Int64) async throws {
        try await UserConnector.userDocumentRef(for: uid).updateData([UserConnector.userScoreKey: FieldValue.increment(on)])
    }
}
