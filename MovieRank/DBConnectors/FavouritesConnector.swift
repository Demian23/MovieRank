...
final class FavouritesConnector {
    ...
    static func favouritesDocumentRef(for uid: String) -> DocumentReference {
        return db.collection(favourites).document(uid)
    }

    static func addMovieForUser(
        movieId id: String, for uid: String, movieProperties prop: FavouritesProperties? = nil
    ) async throws {
    ...
    static func getProperties(movieId id: String, for uid: String) async throws
        -> FavouritesProperties?
    {
        return try await favouritesCollectionRefForUser(for: uid).document(id).getDocument(
            as: FavouritesProperties?.self)
    }

    static func getAllForUserWithLocalLookup(for uid: String, localMovies movies: [Movie])
        async throws -> [Movie]
    {
        var result: [Movie] = []
        ...
        for document in snapshot.documents {
            ...
            if let index = movies.firstIndex(where: { movie in movie.id == movieId }) {
                ...
                guard let fetchedMovie = try await MovieConnector.getMovie(by: movieId) else {
                    continue
                }
