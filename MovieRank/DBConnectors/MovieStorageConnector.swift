...
final class MovieStorageConnector {
        ...
        movieRef.listAll() { result, error in
            ...
            for imageFile in result!.items {
                imageFile.getData(maxSize: 512 * 1024){ data, error in
                    ...
                    guard let data = data else {print("Nothing to fetch");return}
                    guard let image = UIImage(data: data) else {print("Can't construct image");return}
                    completion(image)
                }
            }
        }
    ...
    @MainActor
    static func uploadImages(for movieId: String, images: [UIImage], progressObserver: ImagesUploadProgressObserver){
        let movieRef = st.reference().child(root + "/" + movieId)
        ...
        for i in 0..<images.count{
            ...
            uploadTask.observe(.progress) { snapshot in
                ...
                let progressComplete = Double(progress.completedUnitCount) / Double(progress.totalUnitCount)
