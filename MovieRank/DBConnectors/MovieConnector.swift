...
class MovieConnector {
    ...
    static private let userScoreKey = "marksWholeScore"
    ...
    static func addNewMovie(newMovie movie: Movie, currentUserId uid: String) async throws {
        ...
        if movie.marksAmount > 0 {
            batch.setData(
                [userMarkKey: String(movie.marksWholeScore)],
                forDocument: markDocumentRef(for: movie.id, from: uid))
        }
    ...
    static func updateOrCreateNewMarkForMovie(
        movieId id: String, currentUserId uid: String, newMark mark: String
    ) async throws {
        ...
        let _ = try await db.runTransaction({ (transaction, errorPointer) -> Any? in
            ...
            if !markDocumentSnapshot.exists {
                transaction.updateData(
                    [UserConnector.userScoreKey: FieldValue.increment(Int64(1))],
                    forDocument: userRef)
                transaction.setData([userMarkKey: mark], forDocument: markRef)
                transaction.updateData(
                    [
                        movieMarkComponents[0]: FieldValue.increment(Int64(mark)!),
                        movieMarkComponents[1]: FieldValue.increment(Int64(1)),
                    ], forDocument: movieRef)
                return nil
            } else {
                    ...
                    let error = NSError(
                        ...
                        userInfo: [
                            NSLocalizedDescriptionKey:
                                "Unable to retrieve mark from snapshot \(markDocumentSnapshot)"
                        ]
                    )
                ...
                transaction.updateData(
                    [movieMarkComponents[0]: FieldValue.increment(Int64(mark)! - Int64(oldMark)!)],
                    forDocument: movieRef)
                return nil
            }
        })
    ...
    static func fetchUserMarkForMovie(movieId id: String, currentUserId uid: String) async throws
        -> String
    {
